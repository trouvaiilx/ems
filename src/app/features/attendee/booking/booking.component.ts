                  ...
                  <div class="ticket-selector">
                    ...
                    <div class="quantity-selector">
                      ...
                      <button 
                        ...
                        [disabled]="getSelectedQuantity(ticket) >= ticket.availableTickets || getSelectedQuantity(ticket) >= 10"
                      >
                        +
                      </button>
                    </div>
                  ...
                  <div class="seat-selection">
                    @for (selected of selectedTickets; track selected.ticketType.id) {
                      <div class="seat-group">
                        <h4>{{ getCategoryName(selected.ticketType.category) }} - {{ selected.ticketType.section }}</h4>
                        <div class="seats-grid">
                          @for (seat of generateSeats(selected.ticketType.section, selected.quantity); track seat) {
                            <button 
                              class="seat-btn"
                ...
                <div class="card">
                  ...
                  <div class="promo-section">
                    <input 
                      ...
                      class="promo-input"
                    />
                    <button (click)="applyPromo()" class="btn btn-secondary" [disabled]="validatingPromo">
                  ...
                  @if (promoSuccess) {
                    <p class="success-message">✓ Promo code applied! {{ discountPercentage }}% discount</p>
                  }
                </div>
              }
            </div>
                    ...
                    @for (selected of selectedTickets; track selected.ticketType.id) {
                      <div class="summary-item">
                        <div>
                          ...
                          <p class="item-details">{{ selected.quantity }} x RM {{ selected.ticketType.price }}</p>
                        </div>
...
export class BookingComponent implements OnInit {
  ...
  generateSeats(section: SeatingSection, count: number): string[] {
    const prefix = section === SeatingSection.BALCONY ? 'B' : 
                   section === SeatingSection.MEZZANINE ? 'M' : 'S';
  ...
  getSubtotal(): number {
    return this.selectedTickets.reduce((sum, s) => 
      sum + (s.ticketType.price * s.quantity), 0
    );
  }
  ...
  proceedToPayment(): void {
    ...
    }, user.id, user.fullName, user.email).subscribe({
      next: (booking) => {
